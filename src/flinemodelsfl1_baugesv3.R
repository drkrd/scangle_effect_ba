...
bauges_db <- bauges_db[which(id_placette %in% plotmetsfl1$id_placette), 
...
stopCluster(clus)
registerDoSEQ()
smplstfl1.mix.all <- matrix(unlist(smplstfl1.mix.all), nrow = 50)
...
registerDoParallel(clus, cores = detectCores() - 1)
n <- 5000
baugesfl1.mix.all <- foreach(i = 1:n, .packages=c("dplyr", "data.table", "caret")) %dopar% {
...
ggplot(data=baugesfl1.mdlmets.feu.all[Forest_attr=="Basal area"], aes( y=value, colour=Metrics))+
  geom_freqpoly()+
