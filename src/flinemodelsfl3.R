...
plotmetsfl3 <- rbindlist(plotmetsfl3)
plotmetsfl3 <- plotmetsfl3[, c("m1","m2","m3"):=tstrsplit(meanang,"_",fixed=T),]
plotmetsfl3$m1 <- as.numeric(plotmetsfl3$m1)
plotmetsfl3$m2 <- as.numeric(plotmetsfl3$m2)
plotmetsfl3$m3 <- as.numeric(plotmetsfl3$m3)

plotmetsfl3 <- plotmetsfl3[, m2:= ifelse(is.na(m2), m1, m2),]
plotmetsfl3 <- plotmetsfl3[, m3:= ifelse(is.na(m3), m2, m3),]


plotmetsfl3 <- plotmetsfl3[,class1:=ifelse(m1>=0&m1<10, "a",
                                           ifelse(m1>=10&m1<20, "b",
                                                  ifelse(m1>=20&m1<30, "c",
                                                         ifelse(m1>=30&m1<40,"d","e"))))]
plotmetsfl3 <- plotmetsfl3[,class2:=ifelse(m2>=0&m2<10, "a",
                                           ifelse(m2>=10&m2<20, "b",
                                                  ifelse(m2>=20&m2<30, "c",
                                                         ifelse(m2>=30&m2<40,"d","e"))))]
plotmetsfl3 <- plotmetsfl3[,class3:=ifelse(m3>=0&m3<10, "a",
                                           ifelse(m3>=10&m3<20, "b",
                                                  ifelse(m3>=20&m3<30, "c",
                                                         ifelse(m3>=30&m3<40,"d","e"))))]
plotmetsfl3 <- plotmetsfl3[class1 != "e"]
plotmetsfl3 <- plotmetsfl3[class2 != "e"]
plotmetsfl3 <- plotmetsfl3[class3 != "e"]


plotmetsfl3 <- plotmetsfl3[!m1 %in% c(37.24, 7.16)]
plotmetsfl3 <- plotmetsfl3[!m2 %in% c(37.24, 7.16)]
plotmetsfl3 <- plotmetsfl3[!m3 %in% c(37.24, 7.16)]
...
plotmetsfl3 <- unique(plotmetsfl3[, prob := prop.table(table(cid))[cid], id_placette][])
...
mdls_gfl3 <- foreach(i = 1:n, .packages=c("dplyr", "data.table", "caret")) %dopar% {
  mets_for_model <- plotmetsfl3[, .SD[sample(.N, min(1,.N), prob=wt) ], by = id_placette]
  ...
  x <- (list("modelmets" = list("R2" = model$results$Rsquared, 
             ...
             "index" = which(plotmetsfl3$meanang %in% mets_for_model$meanang)))
  return(x)
}
stopCluster(clus)
registerDoSEQ()
...
mdlmets_gfl3 <- lapply(mdls_gfl3, function(x)
{
  
  ind <- x[["index"]]
  dbase <- plotmetsfl3[ind]
...
mdlmets_gfl3 <- rbindlist(mdlmets_gfl3)
