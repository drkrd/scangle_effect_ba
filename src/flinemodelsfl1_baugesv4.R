...
# classes 'd' and 'e' are not considered in this side.
...
#classification of the values for each flight line into the same classificaiotn as above to frop unwanted data
...
pmetsfl1 <- pmetsfl1[which(id_placette %in% bauges_db$id_placette)]
...

{
  ...
  pmetsfl1.feu.cla <- pmets.withallcls[cl=="a"]
  
  #combine with the remaining plots
  pmetsfl1.feu.cla <- rbind(pmetsfl1.feu.cla, pmets.woallcls)
  ...
  #pick only those pcs and their metrics which belong to class b
  pmetsfl1.feu.clb <- pmets.withallcls[cl=="b"]
  
  #combine with the remaining plots
  pmetsfl1.feu.clb <- rbind(pmetsfl1.feu.clb, pmets.woallcls)
  ...
  #pick only those pcs and their metrics which belong to class b
  pmetsfl1.feu.clc <- pmets.withallcls[cl=="c"]
  
  #combine with the remaining plots
  pmetsfl1.feu.clc <- rbind(pmetsfl1.feu.clc, pmets.woallcls)
...

{
  ...
  #pick only those pcs and their metrics which belong to class a
  pmetsfl1.mix.cla <- pmets.withallcls[cl=="a"]
  
  #combine with the remaining plots
  pmetsfl1.mix.cla <- rbind(pmetsfl1.mix.cla, pmets.woallcls)
  ...
  #pick only those pcs and their metrics which belong to class b
  pmetsfl1.mix.clb <- pmets.withallcls[cl=="b"]
  
  #combine with the remaining plots
  pmetsfl1.mix.clb <- rbind(pmetsfl1.mix.clb, pmets.woallcls)
  ...
  #pick only those pcs and their metrics which belong to class b
  pmetsfl1.mix.clc <- pmets.withallcls[cl=="c"]
  
  #combine with the remaining plots
  pmetsfl1.mix.clc <- rbind(pmetsfl1.mix.clc, pmets.woallcls)
...
##Generate samples######
########################
dbase <- pmetsfl1.con.all
clus <- makeCluster(detectCores() - 1)
registerDoParallel(clus, cores = detectCores() - 1)
smplstfl1.con.all <- foreach(i = 1:10000, .packages=c("dplyr", "data.table", "caret")) %dopar% {
  set.seed(i)
...
stopCluster(clus)
registerDoSEQ()
smplstfl1.con.all <- matrix(unlist(smplstfl1.con.all), nrow = length(unique(dbase$id_placette)))
smplstfl1.con.all <- unique(as.data.table(t(smplstfl1.con.all)))
###########################################################################################################
time_log <- data.frame()
########################################
...
registerDoParallel(clus, cores = detectCores() - 1)
n <- 5000
baugesfl1.con.all <- foreach(i = 1:n, .packages=c("dplyr", "data.table", "caret")) %dopar% {
...
baugesfl1.mdlmets.con.all <- cbind(baugesfl1.mdlmets.con.all, "exp"=rep("all", nrow(baugesfl1.mdlmets.con.all)), "id"=rep(rep(1:5000, 1, each=6), 4))
baugesfl1.mdlmets.con.all <- cbind(baugesfl1.mdlmets.con.all, "fl"=rep("fl1", nrow(baugesfl1.mdlmets.con.all)))

baugesfl1.mdlmets.feu.all <- cbind(baugesfl1.mdlmets.feu.all, "exp"=rep("all", nrow(baugesfl1.mdlmets.feu.all)), "id"=rep(rep(1:5000, 1, each=6), 4))
baugesfl1.mdlmets.feu.all <- cbind(baugesfl1.mdlmets.feu.all, "fl"=rep("fl1", nrow(baugesfl1.mdlmets.feu.all)))

baugesfl1.mdlmets.mix.all <- cbind(baugesfl1.mdlmets.mix.all, "exp"=rep("all", nrow(baugesfl1.mdlmets.mix.all)), "id"=rep(rep(1:5000, 1, each=6), 4))
baugesfl1.mdlmets.mix.all <- cbind(baugesfl1.mdlmets.mix.all, "fl"=rep("fl1", nrow(baugesfl1.mdlmets.mix.all)))
###################################################################################################

ggplot(data=baugesfl1.mdlmets.con.all[Forest_attr=="Basal area"], aes( y=value, colour=Metrics))+
  geom_boxplot()+
